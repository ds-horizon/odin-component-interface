...
if [ "${TEST_MODE}" = "local" ]; then
    export ODIN_DSL_METADATA='{"flavour":"local_docker","stage":"preDeploy","stateConfig":{"provider":"S3","config":{"uri":"s3://'"${S3_BUCKET}"'/odin-component-interface-nginx-test.tfstate","endpoint":"'"${S3_ENDPOINT}"'","region":"us-east-1","forcePathStyle":true}}}'
else
    export ODIN_DSL_METADATA='{"flavour":"local_docker","stage":"preDeploy","stateConfig":{"provider":"S3","config":{"uri":"s3://'"${S3_BUCKET}"'/odin-component-interface-nginx-test.tfstate","endpoint":"'"${S3_ENDPOINT}"'","region":"us-east-1","forcePathStyle":false}}}'
fi
